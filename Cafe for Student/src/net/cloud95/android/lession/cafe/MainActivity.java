...
public class MainActivity extends Activity {
	...
	private void processControl() {
		...
		aa01 = new ArrayAdapter<String>(this,
	...
	class MyOnClickListener implements OnClickListener {
		...
		public void onClick(View view) {
			...
			orders[orderIndex].add(new Cafe(sizeIndex, iceIndex, sugarIndex,
	...
	class MyOnLongClickListener implements OnLongClickListener {
		...
		public boolean onLongClick(View view) {
			...
			tempCafe = new Cafe(sizeIndex, iceIndex, sugarIndex,
					(Integer) view.getTag(),
					Cafe.cafePrice[(Integer) view.getTag()][sizeIndex]);
			int cups = orders[orderIndex].getCafesNumber(tempCafe);
			if (cups <= 0)
				Toast.makeText(MainActivity.this,
						"刪除失敗:查無" + tempCafe.toString(), Toast.LENGTH_LONG)
						.show();
			else if (cups == 1)
				orders[orderIndex].del(tempCafe);
			else {
				AlertDialog.Builder d = new AlertDialog.Builder(
						MainActivity.this);
				d.setTitle(cups + "杯:" + tempCafe.toString());

				String[] items = new String[cups];
			...
			//加入按鈕,事件指定為null值,表示不做任何事情,只會關閉對話框
